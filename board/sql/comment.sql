SELECT * FROM board;
DROP TABLE board;
DROP SEQUENCE board_board_seq;


-- 게시판 테이블 생성
CREATE TABLE board (
    board_id NUMBER(10) PRIMARY KEY,                        --게시글 번호
    title VARCHAR2(100),                                             --게시글 제목
    content CLOB,                                                      --게시글 내용
    writer VARCHAR2(10),                                             --게시글 작성자
    created_date TIMESTAMP DEFAULT SYSTIMESTAMP,      --게시글 작성일
    modified_date TIMESTAMP DEFAULT SYSTIMESTAMP     --게시글 수정일
);
-- 시퀀스 생성
CREATE SEQUENCE board_board_seq;
